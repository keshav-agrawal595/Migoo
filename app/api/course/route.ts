...
import { chapterContent, chapterContentSlides, coursesTable } from "@/config/schema";
...
export async function GET(req: NextRequest) {
    ...
    const slides = await db.select().from(chapterContent).where(eq(chapterContent.courseId, courseId as string));
